...
use Swoopaholic\Component\Table\Exception\LogicException;

/**
 * Renders a table into HTML using a rendering engine.
 *
...
class TableRenderer implements TableRendererInterface
{
    ...
    /**
     * @var TableRendererEngineInterface
     */
    private $engine;
    ...
    public function __construct(TableRendererEngineInterface $engine)
    {
        $this->engine = $engine;
    }
    ...
    public function renderBlock(TableView $view, $blockName, array $variables = array())
    {
        ...
        if (!$resource) {
            throw new LogicException(sprintf('No block "%s" found while rendering the table.', $blockName));
        }
    ...
    public function searchAndRenderBlock(TableView $view, $blockNameSuffix, array $variables = array())
    {
        ...
        //
        // The second kind of hierarchy is the table type hierarchy. This allows
        ...
        if (!$resource) {
            throw new LogicException(sprintf(
                'Unable to render the table as none of the following blocks exist: "%s".',
