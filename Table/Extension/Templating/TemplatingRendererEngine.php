<?php
namespace Swoopaholic\Component\Table\Extension\Templating;

use Swoopaholic\Component\Table\AbstractRendererEngine;
...
class TemplatingRendererEngine extends AbstractRendererEngine
{
     ...
     * {@inheritdoc}
     */
    public function renderBlock(TableView $view, $resource, $blockName, array $variables = array())
    {
     ...
     */
    protected function loadResourceForBlockName($cacheKey, TableView $view, $blockName)
    {
        // Recursively try to find the block in the themes assigned to $view,
        // then of its parent table, then of the parent table of the parent and so on.
        // When the root table is reached in this recursion, also the default
        ...
        // Check the default themes once we reach the root table without success
        if (!$view->parent) {
